...
def main():
    ...
    load_encoder_decoder('./checkpoints/diffusion_frames', '400', motion_enc, motion_dec)
    ...
    load_diffusion('./checkpoints/diffusion_frames', '400', diffusion)

    save_path = './checkpoints/diffusion_frames/result'
    dev = 'cuda:1'
    diffusion.eval()  # 评估模型
    motion_enc.eval()
    motion_dec.eval()
...
def sample_step(test_loader, dev, diffusion, encoder, decoder, epoch_log, epochs, save_folder):
    with tqdm(range(len(test_loader)), desc=f'Sampling [{epoch_log}/{epochs}]') as tbar:
         for n, (audio, motion, template) in enumerate(test_loader):
            ...
            motion_frames = torch.cat([torch.zeros_like(template) for _ in range(1)], dim=1)
