...
from video_diffusion_pytorch.diffusion_adaLN import GaussianDiffusion
...
def main():
    ...
    save_path = './checkpoints/diffusion_vqvae_adaLN'
...
def run_step(epochs, epoch_log, optimizer, train_loader, diffusion, audioencoder, autoencoder, writer, save_path, dev):
    ...
    with tqdm(range(len(train_loader)), desc=f'Train[{epoch_log}/{epochs}]') as tbar:
         for i, (audio, motion, template, one_hot, file_name) in enumerate(train_loader):
            ...
            enc_motion, _ = autoencoder.get_quant(torch.cat([motion, motion, motion], dim=1) - template)  # [batch, 128, 8]

            denoise_loss, result = diffusion(enc_motion[:, :, -8:], audio, one_hot)
            
            # vq_loss = vq_vae_loss(result, enc_motion)
            # feature quantization
            # feat_out_q, _, _ = autoencoder.quantize(torch.cat([result, result, result], dim=2))
            # feature decoding
            ...
            tbar.set_postfix(noise_loss=denoise_loss.item())
            tbar.update(1)
...
def get_audio_motion(all_audio, all_motion, batchsize):
    ...
    audio_idx = torch.cat([idx * 2, idx * 2 + 1], dim=1)

    audio = all_audio[:, audio_idx, :]  # 五帧音频，前三帧对应于面部动作，后两帧维额外信息
    motion = all_motion[:, idx, :]  # 对应的三帧面部运动
