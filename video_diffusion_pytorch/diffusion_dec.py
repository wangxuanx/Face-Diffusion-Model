...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, x_start, t, audio, template, one_hot):

        device = x_start.device
        noise = torch.randn_like(x_start[:, -1:, :]).to(device)  # 对序列的最后一帧加噪声

        # 噪声采样,一次性完成的
        x_noisy = self.q_sample(x_start=x_start[:, -1:, :], t=t, noise=noise)
        x_noisy = torch.cat((x_start[:, :-1, :], x_noisy), dim=1)
    ...
    def forward(self, x, audio, template, one_hot):
        b, device, img_size, = x.shape[0], x.device, self.image_size
        # check_shape(x, 'b c f h w', c = self.channels, f = self.num_frames, h = img_size, w = img_size)
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        # x = normalize_img(x)
        return self.p_losses(x, t, audio, template, one_hot)
