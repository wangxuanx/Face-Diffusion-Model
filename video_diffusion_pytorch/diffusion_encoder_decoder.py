...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, audio, one_hot):
        x_recon = self.denoise_fn(audio, t, x, one_hot)  # gt
    ...
    @torch.inference_mode()
    def sample(self, audio, latent_motion_shape, one_hot):
        return self.p_sample_loop(latent_motion_shape, audio, one_hot)
    ...
    def p_losses(self, x_start, t, audio, emotion_one_hot):
        ...
        # 进入Model中
        x_recon = self.denoise_fn(audio, t, x_noisy, emotion_one_hot)
        # 与gt计算loss
    ...
    def forward(self, x, audio, emotion_one_hot):
        b, device, img_size, = x.shape[0], x.device, self.image_size
        # check_shape(x, 'b c f h w', c = self.channels, f = self.num_frames, h = img_size, w = img_size)
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self.p_losses(x, t, audio, emotion_one_hot)
