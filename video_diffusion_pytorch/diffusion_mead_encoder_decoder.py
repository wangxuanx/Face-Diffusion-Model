...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, audio, emo_one_hot, id_one_hot):
        x_recon = self.denoise_fn(audio, t, x, emo_one_hot, id_one_hot)  # gt
    ...
    def p_sample(self, x, t, audio, emo_one_hot, id_one_hot, clip_denoised = False):
        b, *_, device = *x.shape, x.device
        model_mean, _, model_log_variance = self.p_mean_variance(x, t, clip_denoised, audio, emo_one_hot, id_one_hot)
    ...
    @torch.inference_mode()
    def p_sample_loop(self, shape, audio, emo_one_hot, id_one_hot):
        device = self.betas.device
        b = shape[0]
        img = torch.randn(shape, device=device)
        self.num_timesteps = 1000
        for i in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            img = self.p_sample(img, torch.full((b,), i, device=device, dtype=torch.long), audio, emo_one_hot, id_one_hot)

        return img

    @torch.inference_mode()
    def sample(self, audio, latent_motion_shape, emo_one_hot, id_one_hot):
        return self.p_sample_loop(latent_motion_shape, audio, emo_one_hot, id_one_hot)
    
    @torch.inference_mode()
    def ddim_sample(self, audio, latent_motion, latent_motion_frames, one_hot, steps=10):
        eta = 0.0
        ...
        for i, i_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            # 获得阿尔法和前一阿尔法
            alpha_bar = extract(self.alphas_cumprod, t, motion.shape)
            alpha_bar_next = extract(self.alphas_cumprod, t_next, motion.shape)

            sigma = eta * torch.sqrt((1 - alpha_bar) / (1 - alpha_bar_next)) * torch.sqrt(1 - alpha_bar / alpha_bar_next)

            c = torch.sqrt(1 - alpha_bar_next - sigma ** 2)

            noise = torch.randn(motion.shape, device=device)
    ...
    def p_losses(self, x_start, t, audio, emotion_one_hot, id_one_hot):
        ...
        # 进入Model中
        x_recon = self.denoise_fn(audio, t, x_noisy, emotion_one_hot, id_one_hot)
        # 与gt计算loss
    ...
    def forward(self, x, audio, emotion_one_hot, id_one_hot):
        b, device, = x.shape[0], x.device
        # check_shape(x, 'b c f h w', c = self.channels, f = self.num_frames, h = img_size, w = img_size)
        t = torch.randint(0, self.num_timesteps, (b,), device=device).long()
        return self.p_losses(x, t, audio, emotion_one_hot, id_one_hot)
