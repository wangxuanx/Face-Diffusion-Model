...
class GaussianDiffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, audio, latent_motion_frames, one_hot):
        x_recon = self.denoise_fn.predict(audio, t, x, latent_motion_frames, one_hot)  # gt
    ...
    def p_sample_loop(self, shape, audio, latent_motion_frames, one_hot):
        ...
        for i in tqdm(reversed(range(0, self.num_timesteps - 500)), desc='sampling loop time step'):
    ...
    @torch.inference_mode()
    def ddim_sample(self, audio, latent_motion_shape, one_hot, steps=10):
        eta = 0.0

        shape = latent_motion_shape
        device = self.betas.device
        b = shape[0]
        ...
        for i, i_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            x_recon = self.denoise_fn.predict(audio, t, motion, one_hot)
    ...
    def p_losses(self, x_start, t, audio, one_hot):
        ...
        # 进入Model中
        x_recon = self.denoise_fn(audio, t, x_noisy, one_hot)
        # 与gt计算loss
    ...
    def forward(self, x, audio, one_hot):
        ...
        return self.p_losses(x, t, audio, one_hot)
